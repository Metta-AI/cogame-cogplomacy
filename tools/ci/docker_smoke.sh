...
# Substitute: cogplomacy, coworld-cogplomacy, 7.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-cogplomacy:ci)
#   SMOKE_SLUG                 game slug                        (cogplomacy)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/cogplomacy)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/cogplomacy-player)
#   SMOKE_MANIFEST             manifest template path           (coworld_manifest_template.json)
#   SMOKE_SEATS                seat-count CROSS-CHECK           (7)
...
image="${1:-${SMOKE_IMAGE:-coworld-cogplomacy:ci}}"
slug="${SMOKE_SLUG:-cogplomacy}"
